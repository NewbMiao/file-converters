...
pub struct RunArgs {
    ...
    #[clap(short, long, value_enum, default_value = "weekly")]
    pub s_type: StatType,
...
fn get_default_key() -> &'static OsStr {
    Box::leak(Box::new(OsString::from(get_stat_key(&StatType::Weekly)))).as_os_str()
}

pub fn get_default_migration_name() -> String {
    let date = Local::now().format("%m%d").to_string();
    format!("ProfileStatsSeedBatchIn{}", date)
}
