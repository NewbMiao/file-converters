...
impl CliArgs for RunArgs {
    ...
    fn parse_args_interactively(&mut self) -> Result<Self> {
        let file: String = Input::with_theme(&ColorfulTheme::default())
            .with_prompt("Path of stats file (csv|xlsx)")
        ...
        let do_filter = Input::with_theme(&ColorfulTheme::default())
            .with_prompt(format!(
                "Auto filter base on id_targets.txt? (count: {target_ids_count})"
            ))
            .default(false)
            .interact_text()?;
        let run_post_script = Input::with_theme(&ColorfulTheme::default())
            .with_prompt("Auto run post script?")
            .default(false)
            .interact_text()?;
        Ok(RunArgs {
            file: path,
